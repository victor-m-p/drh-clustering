...
unique_question_mapping = shg_answers[shg_answers["question_id"].isin(unique_questions)]
unique_question_mapping = unique_question_mapping[
    ["question_id", "question_short"]
].drop_duplicates()
unique_question_mapping["question_id"] = "Q" + unique_question_mapping[
    "question_id"
].astype(str)
unique_question_mapping_dict = dict(
    zip(
        unique_question_mapping["question_id"],
...
plt.tight_layout()
plt.savefig(
    f"../figures/{superquestion}_correlation_heatmap.jpg", dpi=300, bbox_inches="tight"
)
