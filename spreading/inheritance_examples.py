...
# 0. setup
question_group = "shg"
question = "permissible to worship other god?"
...
df_values = pd.read_csv(f"../data/preprocessed/{question_group}_long.csv")

# take specific question
df_values = df_values[df_values["question_short"] == question]

# take only consistent values
df_values = df_values[(df_values["answer_value"].notna()) & (df_values["weight"] == 1)]

# take only group polls
df_values = df_values[df_values["poll_name"].str.contains("Group")]

# take out relevant columns
df_values = df_values[["entry_id", "question_short", "answer_value"]]
...
mean_features = mean_features.rename(columns={"index": "question_id"})
mean_features["question_id"] = [int(q[1:]) for q in mean_features["question_id"]]

### merge with questions for interpretability ###
shg_answers = pd.read_csv(f"../data/preprocessed/{question_group}_long.csv")
shg_answers = shg_answers[["question_id", "question_short"]].drop_duplicates()
mean_features = mean_features.merge(shg_answers, on="question_id", how="inner")
...
baseline_acceptance = baseline_acceptance.rename(columns={"index": "question_id"})
baseline_acceptance["question_id"] = [
    int(q[1:]) for q in baseline_acceptance["question_id"]
]

# merge, reorder, and sort
mean_features = mean_features.merge(baseline_acceptance, on="question_id", how="inner")
...
- this just suggests that this does not quite work (missing links?).
"""
...
1. very weak effects. 
2. slightly stronger for more well mixed features.
"""
