...
def load_data(question_short):
    ...
    entry_metrics = pd.read_csv(f"data/{question_short}_metrics.csv")
    entry_data = entry_data[
        ["entry_id", "entry_name", "year_from", "christian_tradition"]
    ]
    df_nodes = entry_data.merge(entry_metrics, on="entry_id", how="inner")
    df_nodes = np.round(df_nodes, 2)  # round all floats

    assert len(df_nodes) == len(entry_metrics)
